// @ts-nocheck
const SENDING_PATH = "/admin/sending";
...
/*------------------------- SMS SOCCKET -------------------------*/
function sendSms() {
    const socket = new WebSocket("ws://localhost:8080/admin/sending/sms");
...
/*------------------------- EMAIL SOCCKET -------------------------*/
function sendEmail() {
    const socket = new WebSocket("ws://localhost:8080/admin/sending/email");
...
$emailFileInput.on("change", async event => {
    ...
    if (file) {
        ...
        fileReader.onload = (readerEvent) => {
            ...
            fetch("/admin/sending/save/email", {
                method: "POST",
                body: formData
            }).then((response) => {
                if(!response.ok) {
