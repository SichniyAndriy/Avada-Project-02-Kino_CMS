...
function gotoTheaters() {
    location.href = PATH_TO_THEATERS;
}

function showAuditorium( audId = 0, theaterId ) {
    location.href = `${ PATH_TO_THEATERS }/show/auditorium/${ audId }/${ theaterId }`;
}

function deleteAuditorium( audId ) {
    fetch( `${ PATH_TO_THEATERS }/delete/auditorium/${ audId }`, {
        method: "DELETE"
    } ).then( responce => {
        if ( responce.ok ) {
...
async function uploadFileToServer( file, name, id ) {
    ...
    const responce = await fetch( `${ PATH_TO_THEATERS }/save/file`, {
        method: "POST",
        body: formData
    } );
